...
public class ZoneWeapon : Weapon
{
    ...
    void Update()
    {
        ...
        if (spawnCounter <= 0f) // If the spawn counter is less than or equal to 0. GK
        {
            spawnCounter = spawnTime; // Reset the spawn counter. GK
            Instantiate(damager, damager.transform.position, Quaternion.identity,transform).gameObject.SetActive(true); // Instantiate the damager at the position of the weapon. GK
        }
    }
    ...
    void SetStats()
    {
        ...
        damager.timeBetweenDamage = stats[weaponLevel].speed; // Set the time between damage of the damager to the speed of the weapon. GK
